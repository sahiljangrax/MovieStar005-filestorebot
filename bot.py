        ...
        if FORCE_SUB_CHANNEL3:
            try:
                ...
                if not link:
                    ...
                    link = (await self.get_chat(FORCE_SUB_CHANNEL3)).invite_link
                self.invitelink3 = link  # Fixed variable name
            except Exception as a:
                self.LOGGER(name).warning(a)
                self.LOGGER(name).warning("Bot can't Export Invite link from Force Sub Channel!")
                self.LOGGER(name).warning(f"Please Double check the FORCE_SUB_CHANNEL3 value and Make sure Bot is Admin in channel with Invite Users via Link Permission, Current Force Sub Channel Value: {FORCE_SUB_CHANNEL3}")
                self.LOGGER(name).info("\nBot Stopped. https://t.me/nextgenbotz for support")
                sys.exit()

        try:  # Corrected indentation for exception handling
